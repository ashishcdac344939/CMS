package in.fridr.entity;

import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.ManyToOne;
import javax.persistence.SequenceGenerator;
import javax.persistence.Table;

import com.fasterxml.jackson.annotation.JsonIgnore;

@Entity
@Table(name = "patient_pain_assesment_pain_master_mapping")
public class PatientPainAssesmentPainMasterMapping {

	@Id
	@SequenceGenerator(name = "patient_pain_assesment_pain_master_mapping_sequence_generator",allocationSize = 1,initialValue = 1)
	@GeneratedValue(strategy = GenerationType.IDENTITY )
	private Integer id;
	
	
	@ManyToOne
	@JoinColumn(name="pain_master_id")
	private PainMaster painMaster;
	@JsonIgnore
	@ManyToOne
	@JoinColumn(name="patient_pain_assesment_id")
	private PatientPainAssessments patientPainAssessments;



	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public PatientPainAssesmentPainMasterMapping() {
		super();
		// TODO Auto-generated constructor stub
	}

	public PatientPainAssesmentPainMasterMapping(PainMaster painMaster, PatientPainAssessments patientPainAssessments) {
		super();
		this.painMaster = painMaster;
		this.patientPainAssessments = patientPainAssessments;
	}

	public PainMaster getPainMaster() {
		return painMaster;
	}

	public void setPainMaster(PainMaster painMaster) {
		this.painMaster = painMaster;
	}

	public PatientPainAssessments getPatientPainAssessments() {
		return patientPainAssessments;
	}

	public void setPatientPainAssessments(PatientPainAssessments patientPainAssessments) {
		this.patientPainAssessments = patientPainAssessments;
	}
	
}
